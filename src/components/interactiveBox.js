...
const InteractiveWindow = (props)=>{
  ...
  return(
      <div className="infoWindow interactivePicGrid">
          ...
          <h2>Animation</h2>
          <p>Bei Stop Motion wird ein
              Bild geschossen, die Figur etwas bewegt und
              danach wieder ein Foto gemacht. Wenn man diese Bilder
              dann schnell hintereinander abspielt, entsteht eine Animation,
              ähnlich wie bei einem Daumenkino.
              Probier's doch einfach gleich mal aus: 
          </p>
          
          <div className={props.btnIsActive ? "btnInteractive" : "btnInteractiveBlocked"} onClick={()=>props.onClick(1)}>
          {props.caption} <br />
          <FaCameraRetro />
          </div>
          <div className="btnInteractive btnRestart" onClick={()=>props.onClick(props.restartStep)}>                
          Neu starten 
          </div>
      </div>
  );
...
<div className="picPanel interactiveVidGrid">
    <h2>Die Animation</h2>
    <video className="interactiveVideo" autoPlay 
      ...
      if(this.state.currentStep % 2 !==0){
        return(
          <div className="interactiveBox"  id="3">
            <InteractiveWindow btnIsActive={this.state.btnIsActive} caption="Warte..." currentStep={this.state.currentStep} restartStep={0} onClick={this.handleClick} />
            <InteractiveVideos onEnded={this.activateButton} videoName={this.getCurrentImg()}/>
        </div>
        );
      }else{
        return(
        <div className="interactiveBox"  id="3">
        <InteractiveWindow btnIsActive={true} caption="Mach ein Foto" currentStep={this.state.currentStep} restartStep={0} onClick={this.handleClick} />
        <InteractivePictures step={this.state.currentStep} data={this.getCurrentImg()}/>
        </div>
    );
      }
    }
...
render={data=>(
    <InteractiveBox props={props} data={data}/>
)}
/>
);
